...
<html lang="zh-CN">
<head>
<title>模板实体管理页面</title>
</head>
<script type="text/javascript">
	$(function () { 
	   	$("#add").click(function(){
	   		parentOpenEdit('添加模板实体','${ctx }/permissions/templateentity/edit.do','1300px','700px')
	   	});
	   	$("#edit").click(function(){
			var $table = $('#table');
	   	 	...
	   	 	parentOpenEdit('编辑模板实体','${ctx }/permissions/templateentity/edit.do?id='+ids,'1300px','700px')
	   	});
		$("#del").click(function(){
            ...
            layer.confirm("删除后数据不可恢复，是否确定删除？", function(){
            	//继续删除
		  		$.ajax({
					type : "post",
					url : "${ctx}/permissions/templateentity/action.do?id="+ids+"&actionName=sc",
	...
	window.actionEvents = {
		...
		'click .get' : function(e, value, row, index) {
			parentOpenGet('查看模板实体','${ctx}/permissions/templateentity/view.do?id='+row.guid,'1300px','600px')
		},
		'click .edit' : function(e, value, row, index) {
			parentOpenEdit('编辑模板实体','${ctx}/permissions/templateentity/edit.do?id='+row.guid,'1300px','700px')
		},
		'click .del' : function(e, value, row, index) {
			layer.confirm("删除后数据不可恢复，是否确定删除？", function(){
            	//继续删除
		  		$.ajax({
					type : "post",
					url : "${ctx}/permissions/templateentity/action.do?id="+row.guid+"&actionName=sc",
					success : function(data) {
						if(data.code==200){
							layer.msg("删除成功", {icon: 1,time: 2000});
						}else{
...
<body>
	...
	<table id="table" data-toggle="table" data-url="${ctx }/permissions/templateentity/queryAllJson.do"
		data-pagination="true" data-side-pagination="server" data-show-refresh="true"
		data-page-list="${PAGE_LIST }" data-search="true"
		data-click-to-select="true" data-show-columns="true" data-method="post"
		...
		<thead>
			<tr>
				...
				<th data-field="guid" data-sortable="true">变量实体id</th>
				<th data-field="entityFullPath" data-sortable="true">变量实体完整路径</th>
				<th data-field="entityComment" data-sortable="true">变量实体注释</th>
